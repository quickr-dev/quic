...
func (s *CheckoutService) CreateCheckout(ctx context.Context, cloneName string) (*CheckoutInfo, error) {
...
func (s *CheckoutService) createZFSClone(config *ZFSConfig, cloneName string) (string, error) {
	...
	if !s.snapshotExists(snapshotName) {
		// Ensure source database is in consistent state before snapshot
		if err := s.prepareSourceForSnapshot(restoreDataset); err != nil {
			return "", fmt.Errorf("preparing source for snapshot: %w", err)
		}

		cmd := exec.Command("sudo", "zfs", "snapshot", snapshotName)
		if err := cmd.Run(); err != nil {
			return "", fmt.Errorf("creating ZFS snapshot: %w", err)
		}
...
`
	...
	// Reset WAL to skip recovery and enable instant startup
	// This is safe for dev clones where we don't need perfect consistency
	pgResetWalCmd := filepath.Join(s.config.PostgresBinPath, "pg_resetwal")
	cmd := exec.Command(pgResetWalCmd, "-f", clonePath)
	if output, err := cmd.CombinedOutput(); err != nil {
		return fmt.Errorf("resetting WAL: %w (output: %s)", err, output)
	}

	// Ensure WAL directory has proper permissions after reset
...
func (s *CheckoutService) updatePostgreSQLConf(confPath string) error {
	...
	cloneSettings := map[string]string{
		...
		"listen_addresses":                "'*'",
		"shared_preload_libraries":        "'pg_stat_statements,pgaudit'", // Load required extensions for clones
		"ssl":                             "on",
...
func (s *CheckoutService) prepareSourceForSnapshot(restoreDataset string) error {
	...
	if !isRunning {
		// If PostgreSQL isn't running, the data is already consistent
		// No need for CHECKPOINT
		return nil
	}

	// Run CHECKPOINT to ensure all dirty buffers are written to disk
	// Use Unix socket connection (more reliable for postgres user)
	psqlCmd := filepath.Join(s.config.PostgresBinPath, "psql")
	socketDir := "/var/run/postgresql"
	checkpointCmd := exec.Command(psqlCmd,
		"-h", socketDir,
	...
	if output, err := checkpointCmd.CombinedOutput(); err != nil {
		return fmt.Errorf("running CHECKPOINT on source database (port %d): %w (output: %s)", port, err, output)
	}
